...
def init(mail):
    ...
    email_url = 'https://api.mail.cx/api/v1/mailbox/{}'
    getAuth = "https://api.mail.cx/api/v1/auth/authorize_token"
    getContent = "https://api.mail.cx/api/v1/mailbox/{}/{}"
    ...
    email_url = email_url.format(mail)
    auth = requests.post(getAuth).json()

    cooki = 'auth_token={}; mtd_address={};'.format(auth,mail)
    ...
    requests.get(email_url, headers=heade)

# 随机数字和字符创建邮箱
def create_email():
    #num=["{}@nqmo.com", "{}@yzm.de","{}@qabq.com","{}@end.tw","{}@uuf.me"]
    #{}@nqmo.com,{}@end.tw疑似不行
    num=["{}@yzm.de","{}@qabq.com","{}@uuf.me"]
    return num[random.randint(0, 1)].format(''.join(random.sample(string.ascii_letters + string.digits, 10)))

def get_emailid(temp):
    # 获取第一个的邮件id
    try:
        emailid = temp.split('id":"')[1].split('","')[0]
        return emailid
    except:
        return False
...
def get_verification_code():
    while True:
        req=requests.get(email_url, headers=head)
        id = get_emailid(req.text)
        if id:
            response = requests.get(getContent.format(em, id), headers=head)
            data = json.loads(response.text)  # 使用Response对象的text属性
...
#是否成功增加
def is_success(temp):
    gettext=temp['popup']["title"]['text']
    if "你获得5天免费会员" in gettext:
        print("邀请成功")
...
def start(code):
    ...
    temp=part_invite(action9['sub'], phoneModel, phoneBuilder, invite_code, action10['captcha_token'], device_id,
                action9['access_token'], User_Agent)
    

    temp2=is_success(temp) 
    print("Email：", email)
    print("PWD：", password)
    return temp2
...
if __name__ == '__main__':
    ...
    #num=input("请输入次数(最高50)：")
    print('注意事项：')
    print("默认3次，单ip多次容易收回会员")
    print('')
    print("免责声明")
    print('1. 目的仅限测试和学习. 为代码初学者提供帮助.')
    print('2. 本项目不为任何形式的使用和传播负责. 你必须在测试或注册 24 小时内主动删除数据.')
    print('3. 本项目不对任何使用后产生的后果或负面影响负责.')
    print('4. 禁止利用本项目盈利. 禁止利用本项目为个人带来非法或不合规的利益.')
    print('5. 所有使用本项目的用户, 需遵守当地法律法规, 本项目不对用户的任何行为负责.')
    print('6. 对于不遵守本声明或违反其他条款的用户, 本项目有权随时中止或终止其服务, 且不承担任何责任.')
    print('')
    print('作者:B站-纸鸢花的花语')
    print('二改:QQ-干嘛')
    print('三改:QQ-反干扰西瓜')
